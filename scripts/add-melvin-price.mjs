...
while ((match = schemaRe.exec(html))) {
  try {
    ...
    if (prior) {
      prior.url = url;
      prior.name = 'Melvin Price Live: Mississippi Tow & Lock Traffic';
      changed = true;
    } else {
...
if (!html.includes(`href="${url}"`)) {
  ...
  html = html.replace(ballardIntent, `${ballardIntent}<li><a href="${url}">Time a Melvin Price Locks visit<span>Live tow queue, lock activity, Mississippi flow and stage, AIS vessels, weather and free tour timing</span></a></li>`);
