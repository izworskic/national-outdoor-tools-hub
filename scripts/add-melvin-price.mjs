...
const buildMarker = 'melvin-national-card-v3';
...
// Rebuild each Melvin surface independently on every build. This prevents a surviving
// intent link from causing the featured/library cards to disappear on a later deploy.
html = html.replace(/<li><a href="https:\/\/chrisizworski\.com\/(?:melvin-price\/|national-tools\/melvin-price-live\/)">Time a Melvin Price Locks visit[\s\S]*?<\/li>/g, '');
...
const intent = `<li><a href="${url}">Time a Melvin Price Locks visit<span>Live tow queue, lock activity, Mississippi flow and stage, AIS vessels, weather, free tour timing and a go-now visitor outlook</span></a></li>`;
html = html.replace(ballardIntent, `${ballardIntent}${intent}`);
