...
test('regional organizer preserves the existing directory while separating national and regional tools',()=>{
  ...
  for(const script of ['sync-thunder-hole-discovery.mjs','sync-rockies-expansion.mjs']){
    const sync=spawnSync(process.execPath,[path.join(root,'scripts',script)],{
      cwd:root,
  ...
  for(const region of ['Northeast & Great Lakes','Appalachia & Ohio Valley','Southeast','Mississippi & Great Plains','Rockies','Pacific Northwest']){
