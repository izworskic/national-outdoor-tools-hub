...
const elk = require("../api/elk-rut-live.js")._test;
...
test("Moraine Park uses the Bear Lake Road timed-entry window", () => {
  ...
  const inside = new Date("2026-09-11T23:30:00Z");
  const outside = new Date("2026-09-12T00:30:00Z");
...
test("weather score rewards quieter and drier viewing conditions", () => {
  const calm = elk.weatherQuality([{ temperature: 44, windSpeed: "5 mph", probabilityOfPrecipitation: { value: 5 }, shortForecast: "Clear" }]);
  const rough = elk.weatherQuality([{ temperature: 58, windSpeed: "30 mph", probabilityOfPrecipitation: { value: 80 }, shortForecast: "Thunderstorms" }]);
