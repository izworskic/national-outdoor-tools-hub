...
test('Trail Ridge uses official NPS road wording as the controlling status',()=>{
  ...
  assert.equal(open.label,'Open to through travel');
  assert.match(open.timedEntry,/Timed Entry Reservations/i);

  const closed=trail.roadStatus('Trail Ridge Road is Closed for the Season. Weather conditions vary.');
  assert.equal(closed.level,'closed');
  assert.equal(closed.label,'Closed for the season');
});

test('Trail Ridge weather assessment remains supporting context, not road authority',()=>{
...
test('Yellowstone prediction selector drops expired forecasts and prefers official attribution',()=>{
  ...
  const grand=selected.find(x=>x.geyserName==='Grand');
  assert.equal(oldFaithful.source,'National Park Service');
  assert.equal(oldFaithful.available,true);
  assert.equal(grand.available,false);
});

test('Yellowstone soonest returns an actual prediction window and never manufactures one',()=>{
  const now=Date.parse('2026-09-16T18:00:00Z');
  const items=[
  ...
  const next=geysers.soonest(items,now);
  assert.equal(next.geyserName,'Old Faithful');
  assert.equal(next.windowOpen,'2026-09-16T18:20:00Z');
});

test('Rockies pages expose all three distinct decisions and their source boundaries',()=>{
